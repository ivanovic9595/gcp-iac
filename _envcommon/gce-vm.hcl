...
inputs = {
  ...
  region              = local.region
  num_instances       = 1
  # static_ips          = [dependency.ip.outputs.addresses[index(dependency.ip.outputs.names, lower("${local.name-prefix}-${local.name}"))]]
  ...
  access_config = [{
    nat_ip       = dependency.ip.outputs.addresses[index(dependency.ip.outputs.names, lower("${local.name-prefix}-${local.name}"))]
